...
This is the repo for the sz20 emulator (and hopefully real hardware that will come one day !). Emulator? Well, since sz2 hw is delayed, i started working on an emulator environment, that would cover basic functionality for the upcoming product. It's written mostly in python and using gst for the video pipeline. At some point we decided to use gstreamer for the realtime video feed from the device. I double-checked that both iOS and Android have supporting framework for that. gstreamer1 seems to be a lot better than gstreamer-0.1 which was used in Jasmine (MYS02) back in the days.

This is how the project evolved over time:

* set up CHIP for gstreamer
* write python server
* add commands via GET 
* support video stream and single image snapshot
* add support for scan sequence (several images in a row)
* write configuration for light, camera, and other device caps
* test gst.c and gst python bindings
...
At the time of writing this manual there was no real sz20 hardware available. The entire ecosystem described in this manual is developed in a form of emulator hardware/software. The endpoints such as peripherals for imaging, wifi, ble etc. are all either emulated or completely omitted. Here is the list of all subsystems that were successfully emulated:
...
* Camera : using gstreamer CLI
    ...
    * http 
    * tcp sockets via python (currently unused, since we use http / REST)


## What you need in order to set up the emulating device (CHIP or RPI)
...
As explained earlier, this version of firmware is intended for sz20 emulation. Emulator can be any Mac or linux machine. I use e.g. NextThing CHIP with embedded linux as sz20 emulator. You can also run this software on your Mac equally following the instructions.


## CHIP users only: connect CHIP to your local WiFi

CHIP that you got from me, the author, is already all set up for emulation. Just follow these instructions to do one final setup, ie. connection to your local SSID, and you are ready to go.


* This is a one time configuration, do it once, and after that CHIP will automatically establish connection to your home router every time you power it up.
* Connect CHIP using OTG USB 2.0 cable to your Mac (or Linux box, sorry I don't use Windows, you can probably do the same using Putty).
* Wait one minute, to make sure CHIP OS is up and running
* In terminal start screen to login to chip via USB COM:


    # you should see chip as /dev/tty.usbmodemxxxx
    screen /dev/tty.usbmodem1413 
    
* at that point you will be prompted for username and pass to login. Type 'root' for user, and 'chip' for password:
...
* list all visible SSIDs to make sure your local one is visible too:


    nmcli device wifi list = shows all wifi networks
    
* finally, add a new SSID / PSK record so that CHIP can connect to your local SSID:

    
    # Note: don't skip '' around ssid and psk!!
    # After chip establishes connection, it will store it permanently, and each time you power up CHIP at home, it will automatically connect to your home router
    sudo nmcli device wifi connect '(your wifi network name/SSID)' password '(your wifi password)' ifname wlan0


That's it. Your CHIP is ready to use. Each time you power it up, you will need to wait half a minute or minute until wifi is ready.



## Working with Emualtor
...
To set a resolution, call this before starting video stream.
    ...
    curl chip.local:8001?act=play&host=myself.local&port=5000
    # .. after a while
    curl chip.local:8001?act=stop
...
## Make a developer friendly environment without too much coding

Options:

* cli tool, with a menu
* python + GUI (tkinter wxwidgets or similar)
* webpage (sexy one)

Let's start with CLI tool.